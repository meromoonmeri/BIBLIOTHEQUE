...
module RandomDungeon
  ...
  class Dungeon
    ...
    def get_random_free_tile(map) # unused
      success = false
      x = 0
      y = 0
      100.times do
        x = rand(map.width)
        y = rand(map.height)
        next if @map_data.value(x, y) != :room
        success = true
        break
      end
      if !success
      ...
      1000.times do
        ...
        event_width.times do |i|
          event_height.times do |j|
            ...
            # Additional check: the tile must be passable
...
EventHandlers.add(:on_game_map_setup, :random_dungeon,
  proc { |map_id, map, _tileset_data|
    ...
    # echoln params.instance_variables
    # =================================================================
    ...
    #params.instance_variable_set(:@room_max_width, 10)
    # =================================================================
    ...
    # puts dungeon.write
    $PokemonGlobal.saveDungeonLayout = dungeon.write # elena
    # echoln dungeon.write

    # =================================================================
    # creates events
    createPokemonEventsDungeon(complexity, map, 9)

    # print to verify
    map.events.each do |id, event|
      echoln [id, event.name]
    end
    # =================================================================
=begin
    failed = false
    ...
    if failed
      ...
      echoln _INTL("Couldn't place all events and the player in rooms.")
    end
=end
  failed = false
  occupied_tiles = []
  ...
  100.times do
    ...
    # Separate item events and others
    map.events.each_value do |event|
    ...
    # Place item events (optional: allow overlap with other items if needed)
    placed_items = []
    ...
    if tile
      ...
      # If no space for player, remove an item and use its tile
      echoln "replacing item with player"
      if placed_items.any?
  ...
  if !player_placed
    echoln _INTL("Couldn't place the player in any room tile. They will be placed at the stairs.")
    #coordinates = dungeon.get_random_player_tile(map)
    #$game_temp.player_new_x         = coordinates[0]
    #$game_temp.player_new_y         = coordinates[1]
    map.events.each_value do |event|
  ...
  # ---------------------------------------------------------------------------
  # Ensure player tile is passable
  # ---------------------------------------------------------------------------
  px = $game_temp.player_new_x
  py = $game_temp.player_new_y

  if !map.passable?(px, py)
    echoln "Player spawned on blocked tile (#{px}, #{py}), moving to stairs."
    ...
    if stairs_tile
      ...
      echoln "No stairs found as fallback!"
    end
  end
