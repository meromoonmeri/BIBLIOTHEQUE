...
class MoveSelectionSprite < Sprite
  ...
  def refresh
    ...
    self.x += RELEARNER_UI_X if @fifthmove
    self.y += RELEARNER_UI_Y if @fifthmove
...
class PokemonSummary_Scene
  ...
  def pbStartScene(party, partyindex, inbattle = false)
    ...
    @viewportBG.z = 99999
    @viewport = Viewport.new(SUMMARY_UI_X, SUMMARY_UI_Y, Graphics.width, Graphics.height)
    @viewport.z = 99999
    @party      = party
  ...
  def drawPageOne
    overlay = @sprites["overlay"].bitmap
    base   = Color.new(248, 248, 248)
    shadow = Color.new(0, 0, 0, 120) # Color.new(104, 104, 104)
  ...
  def drawPageTwo
    ...
    for evoData in @data.get_evolutions
      if PokemonPokedexInfo_Scene::SINGLE_EVOLUTION_METHOD && lastEvolutionSpecies==evoData[0]
    ...
    for string in evolutionsStrings
      line=0
      memo += "\n"
      memo += red_text_tag + _INTL("See all possible evolutions in this Pokémon's Pokédex page.")
      break
    end
=begin
...
=end
  ...
  def getEvolutionMessage(evolution, method, parameter)
    ...
    ret = case method
      ...
      when :LevelDay
        _INTL("{1} by leveling during the day",evoName)
      when :LevelNight
        _INTL("{1} by leveling during the night",evoName)
      when :Item
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    if move_to_learn
      @sprites["overlay"].x = SUMMARY_UI_X
      @sprites["overlay"].y = SUMMARY_UI_Y
    end
  end
  ...
  def drawSelectedMove(move_to_learn, selected_move)
    ...
    if move_to_learn
      @sprites["pokeicon"].x       = 46 + SUMMARY_UI_X
      @sprites["pokeicon"].y       = 92 + SUMMARY_UI_Y
    end
    textpos = []
