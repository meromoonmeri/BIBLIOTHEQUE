...
class Spriteset_Map
  ...
  def update
    if @black_overlay && !$game_temp.camera_offset.nil?
      shake_x, shake_y = $game_temp.camera_offset
      @black_overlay.ox = (@map.display_x / Game_Map::X_SUBPIXELS).round + shake_x
      @black_overlay.oy = (@map.display_y / Game_Map::Y_SUBPIXELS).round + shake_y
    end
    ...
    stillPanoramas = ["eclipsePan"]
    @panorama.ox = stillPanoramas.include?(@panorama_name) ? tmox : tmox / 2
    @panorama.oy = tmoy / 2
