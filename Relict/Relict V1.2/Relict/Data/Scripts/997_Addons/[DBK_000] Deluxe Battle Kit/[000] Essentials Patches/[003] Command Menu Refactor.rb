...
class Battle::Scene::FightMenu < Battle::Scene::MenuBase
  def initialize(viewport, z)
    ...
    if USE_GRAPHICS
      ...
      @typeBitmap    = AnimatedBitmap.new(_INTL("Graphics/UI/category"))
      ...
      @typeIcon.x      = self.x + 416 + WIDTH_MOVE_BUTTONS
      ...
      @shiftButton.x      = self.x + 4 + WIDTH_MOVE_BUTTONS
  ...
  def refreshButtonNames
    ...
    @buttons.each_with_index do |button, i|
      ...
      textPos.push([moves[i].short_name, x, y, :center, base, shadow, true])
    end
...
class Battle::Scene
  ...
  def pbCommandMenu(idxBattler, firstAction)
    ...
    cmds = [
      _INTL("What will {1} do?", @battle.battlers[idxBattler].name),
