...
class Battle::Scene::CommandMenu < Battle::Scene::MenuBase
  ...
  def initialize(viewport, z)
    ...
    @msgBox = Window_UnformattedTextPokemon.newWithSize(
      "", self.x + 16, self.y + 32, 400, Graphics.height - self.y, viewport # elena
    )
    ...
    if USE_GRAPHICS
      ...
      @buttonBitmap = AnimatedBitmap.new(_INTL("Graphics/UI/Battle/cursor_command" + spanish))
...
class Battle::Scene::FightMenu < Battle::Scene::MenuBase
  ...
  def refreshMoveData(move)
    ...
    # Type icon
    type_number = move.display_category(@battler) # elena
    @typeIcon.src_rect.y = type_number * TYPE_ICON_HEIGHT

    textPos = []
    # physical special text
    movetext = move.power == 0 ? _INTL("Power: ---") : _INTL("Power: {1}", move.power)
    textPos.push([movetext,
                  64, 24, :center, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR])
    textPos.push([_INTL("Acc: {1}", move.accuracy),
                  64, 56, :center, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR])     
    # PP text
    if move.total_pp > 0
      ppFraction = [(4.0 * move.pp / move.total_pp).ceil, 3].min
      textPos.push([_INTL("PP: {1}/{2}", move.pp, move.total_pp),
                    448 + WIDTH_MOVE_BUTTONS, 56, :center, PP_COLORS[ppFraction * 2], PP_COLORS[(ppFraction * 2) + 1]])
