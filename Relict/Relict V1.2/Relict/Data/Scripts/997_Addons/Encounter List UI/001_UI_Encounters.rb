...
class EncounterList_Scene
  ...
  def initialize
    ...
    if @encounter_data
      @encounter_tables = $PokemonGlobal.dungeonEncounters # Marshal.load(Marshal.dump(@encounter_data.types))
  ...
  def drawPresent
    ...
    loctext = _INTL("<ac><c3=e4c255ff>{1}: {2}</c3></ac>", $game_map.name,name)
    loctext += sprintf("<al><c3=e4c255ff>Total encounters for area: %s</c3></al>",enc_array.length)
    loctext += sprintf("<c3=e4c255ff>-----------------------------------------</c3>")
    @sprites["locwindow"].setText(loctext)
  end
  
  # Draw text if map has no encounters defined (e.g. in buildings)
  def drawAbsent
    loctext = _INTL("<ac><c3=e4c255ff>{1}</c3></ac>", $game_map.name)
    loctext += sprintf("<al><c3=e4c255ff>This area has no encounters!</c3></al>")
    loctext += sprintf("<c3=e4c255ff>-----------------------------------------</c3>")
    @sprites["locwindow"].setText(loctext)
  end
