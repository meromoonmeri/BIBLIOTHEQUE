  ...
  def drawItem(index, _count, rect)
    ...
    if index == self.itemCount - 1
      ...
      if @sorting && index == self.index
        ...
        baseColor   = Color.new(40, 40, 40)
        shadowColor = Color.new(140, 130, 120)
      end
      textpos.push(
...
class PokemonBagPartyPanel < Sprite
  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      outlineColor = Color.new(40, 40, 40) # Color.new(0, 0, 0)
...
class PokemonBag_Scene
  ...
  ITEMLISTSHADOWCOLOR    = Color.new(40, 40, 40)# Color.new(157, 157, 167)
  ITEMTEXTBASECOLOR      = Color.new(239, 239, 239)
  ITEMTEXTSHADOWCOLOR    = ITEMLISTSHADOWCOLOR
  POCKETNAMEBASECOLOR    = Color.new(255, 255, 255)
  POCKETNAMEOUTLINECOLOR = ITEMLISTSHADOWCOLOR # Color.new(78, 83, 100)
  ITEMSVISIBLE           = 6

  def pbUpdate
    pbUpdateSpriteHash(@sprites)
    @sprites["panorama"].x  = 0 if @sprites["panorama"].x == - 56
    @sprites["panorama"].x -= 1 if BagScreenWiInParty::PANORAMA == true
  end

  def pbStartScene(bag, party, choosing = false, filterproc = nil, resetpocket = true)
    @viewportBG    = Viewport.new(0, 0, Graphics.width, Graphics.height) # elena
    @viewportBG.z  = 99999 # elena

    @viewport   = Viewport.new(BAG_UI_X, BAG_UI_Y, Graphics.width, Graphics.height)
    @viewport.z = 99999

    @viewportCMD    = Viewport.new(0, 0, Graphics.width, Graphics.height) # elena
    @viewportCMD.z  = 99999 # elena
    ...
    @sprites = {}
    @sprites["background"] = IconSprite.new(0, 0, @viewportBG)
    @sprites["background"].setBitmap("Graphics/UI/Bag Screen with Party/bg")
    # @sprites["gradient"] = IconSprite.new(0, 0, @viewportBG)
    # @sprites["gradient"].setBitmap("Graphics/UI/Bag Screen with Party/grad")
    @sprites["panorama"] = IconSprite.new(0, 0, @viewportBG)
    ...
    @sprites["itemicon"] = ItemIconSprite.new(48, Graphics.height - 46 - BAG_UI_Y * 2, nil, @viewport)
    pbAddSpriteOutline(["itemicon", @viewport, nil, PictureOrigin::CENTER])
    pbColorOutline("itemicon", Color.new(240, 240, 240))
    @sprites["itemtext"] = Window_UnformattedTextPokemon.newWithSize(
      "", 72, 274, Graphics.width - 72 - 24 - BAG_UI_X * 2, 128, @viewport
    )
    ...
    @sprites["helpwindow"].viewport = @viewportCMD
    pbBottomLeftLines(@sprites["helpwindow"], 1)
    @sprites["msgwindow"] = Window_AdvancedTextPokemon.new("")
    @sprites["msgwindow"].visible  = false
    @sprites["msgwindow"].viewport = @viewportCMD
  ...
  def pbRefreshIndexChanged
    ...
    pbUpdateOutline("itemicon", itemlist.item)
