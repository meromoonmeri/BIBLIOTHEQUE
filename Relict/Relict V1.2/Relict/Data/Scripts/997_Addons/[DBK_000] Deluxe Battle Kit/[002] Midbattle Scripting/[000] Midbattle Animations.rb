...
class Battle::Scene
  ...
  # elena arceus stuff
  def cycleBattleBackground(delay = 0, fade_speed = 15)
    bg_files = (0..33).map { |i| "powerpoint_#{i}_bg" }
    battleBG = @sprites["battle_bg"]

    @current_bg_index ||= 0
    @current_bg_index = (@current_bg_index + 1) % bg_files.length
    next_bg = bg_files[@current_bg_index]

    while battleBG.opacity > 0
      battleBG.opacity -= fade_speed
      Graphics.update
    end

    battleBG.setBitmap("Graphics/Battlebacks/ArceusBGs/" + next_bg)

    while battleBG.opacity < 255
      battleBG.opacity += fade_speed
      Graphics.update
    end
