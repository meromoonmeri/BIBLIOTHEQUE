...
def pbDisplayNametagWindow(msgwindow,name)
  name = "<ac><fs=#{MessageConfig::NARROW_FONT_SIZE}>" + name + "</fs></ac>"
  namewindow=Window_AdvancedTextPokemon.new(_INTL("{1}",name),MessageConfig.pbGetNarrowFontName)
  namewindow.windowskin = nil
  namewindow.baseColor=Color.new(228,194,85)
  namewindow.shadowColor=Color.new(0,0,0,255)
  namewindow.width=msgwindow.width
  if msgwindow.y==0
...
def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil)
  ...
  ### Elena edits
  msgwindow.z += 2 # if $game_temp.in_battle
  # text = "<ac>" + text + "</ac>"
  text = "<ac><c3=FFFFFF,000000FF>" + text + "</c3></ac>"
  # text = "<ac><c3=FFFFFF,000000FF><fn=NDS12><fs=23>" + text + "</fs></fn></c3></ac>"
  # text = "<ac>" + text + "</ac>" if !text.include?("tg[")
  linecount = 2
  textboxpic = Sprite.new(msgwindow.viewport)
  textboxpic.bitmap = RPG::Cache.picture("frame1")
  textboxpic.z = msgwindow.z - 1

  msgwindow.windowskin = nil
  msgwindow.width = 540
  msgwindow.height = 220
  msgwindow.x = 50 # (640 - 100) /2
  ###
  ...
  textchunks = []
  controls = []
  while text[/(?:\\(f|ff|ts|cl|me|se|wt|wtnp|ch|tg)\[([^\]]*)\]|\\(g|cn|pt|wd|wm|op|cl|wu|\.|\||\!|\^))/i]
  ...
  atTop = (msgwindow.y == 0)
  # Show text
  Mugshot.show(msgwindow) # Elena
  msgwindow.y = 362 # Elena
  msgwindow.text = text
  loop do
    ...
    controls.length.times do |i|
      ...
      when "tg"     # Display nametag by polectron
        ...
        ### Elena edits
        textboxpic.bitmap = RPG::Cache.picture("frame0")
        ###
    ...
    if !msgwindow.pausing? && msgwindow.busy? && !msgwindow.text.include?("(...)")# Elena
      Mugshot.showMouth
    else
      Mugshot.hideMouth
    end
  ...
  Mugshot.hideMouth if commandProc
...
def pbShowCommands(msgwindow, commands = nil, cmdIfCancel = 0, defaultCmd = 0)
  cmdIfCancel = 0 if cmdIfCancel < 0
  ...
  if !Mugshot.running?
    pbPositionNearMsgWindow(cmdwindow, msgwindow, :right) # elena
  else
    cmdwindow.x = 0; cmdwindow.y = 0 # elena
    cmdwindow.width = Graphics.width if cmdwindow.width > Graphics.width # elena
  end
