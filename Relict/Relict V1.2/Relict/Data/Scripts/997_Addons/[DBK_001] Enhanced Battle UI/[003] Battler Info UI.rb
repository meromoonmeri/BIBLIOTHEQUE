...
class Battle::Scene
  ...
  def pbUpdateBattlerInfo(battler, effects, idxEffect = 0)
    ...
    textPos  = [
      ...
      [_INTL("Turn {1}", @battle.turnCount + 1), 512 - 32 - xpos, ypos + 8, :center, BASE_DARK, SHADOW_DARK]
    ]
    #---------------------------------------------------------------------------
    # Battler icon.
    @battle.allBattlers.each do |b|
      @sprites["info_icon#{b.index}"].x = iconX + BATTLE_UI_X_POS
      @sprites["info_icon#{b.index}"].y = iconY + BATTLE_UI_Y_POS
