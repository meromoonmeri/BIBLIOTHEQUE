...
module VisibleEncounterSettings
  ...
  Add_Steps_Before_Vanish = [ # default
    [:shiny?, true, 8]       # [:shiny, true, 8]       - means that spawned shiny pokemon will more 8 steps longer on the map than default.   # [:species, :PIDGEY, -2] - means that pidgeys will be gone faster (2 steps earlier).
  ]
...
class Game_Map
  def spawnPokeEvent(x,y,pokemon)
    ...
    event.pages[0].graphic.character_hue = pokemon.hue if pokemon.applicable_hue?
...
def pbCheckBattleAllowed()
  ...
  return false if $PokemonGlobal && $PokemonGlobal.surfing
