...
class Battle::Scene
  ...
  def pbToggleBallInfo(idxBattler)
    ...
    @sprites["enhancedUI_BG"].visible = !@enhancedUIToggle.nil?
  ...
  def pbUpdateBallSelection(items, index, showDesc = false)
    ...
    ypos = @sprites["messageBox"].y - 128 - POKEBALL_UI_MARGIN
    imagePos = [[@path + "pokeball_bg", 0, ypos]]
    imagePos.push([@path + "pokeball_desc", 0, ypos - 69]) if showDesc
    # ==========================
    pbDrawImagePositions(@enhancedUIOverlay_BG, imagePos)
    imagePos = []
    # ==========================
    ...
    textPos = [
      ...
      [action, Graphics.width - 46 - BATTLE_UI_X_POS*2, textY, :center, BASE_LIGHT],
      [name, Graphics.width / 2 - BATTLE_UI_X_POS, textY, :center, BASE_LIGHT, SHADOW_LIGHT, :outline]
    ]
    ballY = @sprites["messageBox"].y - 25 - POKEBALL_UI_MARGIN
    ...
    range.each_with_index do |pos, i|
      if pos < 0 || pos > items.length - 1
        ...
        if try_item
          ...
          textPos.push([items[pos][1].to_s, x - BATTLE_UI_X_POS, ballY, :center, *text_colors])
        end
      end
    end
    pbDrawImagePositions(@enhancedUIOverlay, imagePos)
    pbDrawTextPositions(@enhancedUIOverlay, textPos)
    drawTextEx(@enhancedUIOverlay, 10, ypos - 21, 512 - 10, 2, 
  ...
  def pbSelectBallInfo(idxBattler, pocket)
    ...
    @sprites["leftarrow"].x = 174 + BATTLE_UI_X_POS
    @sprites["leftarrow"].y = @sprites["ball_icon0"].y
    @sprites["rightarrow"].x = 298 + BATTLE_UI_X_POS
