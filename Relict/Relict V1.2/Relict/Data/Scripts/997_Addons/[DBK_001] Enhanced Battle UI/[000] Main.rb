...
module Settings
  ...
  #-----------------------------------------------------------------------------
  USE_MOVE_TYPE_BACKGROUNDS = false
...
class Battle::Scene
  ...
  SHADOW_DARK    = Color.new(140, 130, 120)
  #-----------------------------------------------------------------------------
  ...
  def pbInitSprites
    ...
    if !pbInSafari?
      ...
      @sprites["enhancedUI_BG"] = BitmapSprite.new(Graphics.width, Graphics.height, @viewport)
      @sprites["enhancedUI"] = BitmapSprite.new(Graphics.width, Graphics.height, @viewport)
      # ==========================
      @sprites["enhancedUI_BG"].z = 300
      @sprites["enhancedUI_BG"].visible = false
      @sprites["enhancedUI_BG"].y = BATTLE_UI_Y_POS
      # ==========================
      @sprites["enhancedUI"].x = BATTLE_UI_X_POS
      @sprites["enhancedUI"].y = BATTLE_UI_Y_POS
      ...
      # ==========================
      @enhancedUIOverlay_BG = @sprites["enhancedUI_BG"].bitmap
      # ==========================
      @sprites["plate"] = BitmapSprite.new(Graphics.width, Graphics.height, @viewport)
      @sprites["plate"].x = -@sprites["plate"].bitmap.width
      @sprites["plate"].y = 124
      @sprites["plate"].z = 300
      @sprites["plate"].visible = false
      @plateOverlay = @sprites["plate"].bitmap
      pbSetSmallFont(@sprites["plate"].bitmap)
      imagePos = [
      ...
      @sprites["plate"].opacity = shouldShowArcyPlatesIcon ? 255 : 0
      # ==========================
      ...
      @battle.allBattlers.each do |b|
        ...
        @sprites["info_icon#{b.index}"].x += BATTLE_UI_X_POS
        @sprites["info_icon#{b.index}"].y += BATTLE_UI_Y_POS
      ...
      ballY = @sprites["messageBox"].y - 56 - POKEBALL_UI_MARGIN
      5.times do |i|
        case i
        ...
        @sprites["ball_icon#{i}"] = ItemIconSprite.new(ballX + BATTLE_UI_X_POS, ballY + BATTLE_UI_Y_POS, nil, @viewport)
  ...
  def pbFightMenu_Extra(*args)
    ...
    elsif Input.trigger?(Input::SPECIAL) && shouldShowArcyPlatesIcon
      pbActivateArcyPlates
    end
  end
...
class Battle::Scene::Animation::EnhancedUIPromptAppear < Battle::Scene::Animation
  def createProcesses
    ...
    prompt = addSprite(@sprites["plate"])
