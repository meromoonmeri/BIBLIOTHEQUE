...
module Input
  class << self
    unless method_defined?(:speedup_update)
      alias_method :speedup_update, :update
    end
    ...
    def update
      ...
      if $CanToggle && trigger?(Input::ALT)
        ...
        $speedChange += (System.unscaled_uptime * SPEEDUP_STAGES[-1])
...
module System
  ...
  def self.uptime
    return SPEEDUP_STAGES[$GameSpeed] * unscaled_uptime + $speedChange
  end
end
#===============================================================================#
...
def pbResetInterpreterWaitCount
  @interpreter.pbRefreshWaitCount if @interpreter && (@trigger == 4 || @trigger == 3)
end
end  
...
module Graphics
  class << self
    ...
    def set_button_bitmap
      bitmap_path = $GameSpeed == 0 ? "Graphics/Pictures/FastForward" : "Graphics/Pictures/FastForward1"
