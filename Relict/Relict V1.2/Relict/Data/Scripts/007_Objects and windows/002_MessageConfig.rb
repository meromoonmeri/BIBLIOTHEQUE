...
module MessageConfig
  ...
  LIGHT_TEXT_SHADOW_COLOR  = Color.new(0, 0, 0) # Color.new(72, 80, 88)
  DARK_TEXT_MAIN_COLOR     = Color.new(80, 80, 88)
  DARK_TEXT_SHADOW_COLOR   = Color.new(160, 160, 168)
  DARK_TEXT_SHADOW_COLOR_ALT =  Color.new(0, 0, 0) # Color.new(80, 80, 88) # elena
  ...
  def self.pbDefaultSystemFontName
    return MessageConfig.pbTryFonts("Noto Sans SC") if $PokemonSystem.language == 2
...
def pbPositionNearMsgWindow(cmdwindow, msgwindow, side)
  return if !cmdwindow
  if msgwindow
    heightCMD = 348 # elena, initially replaced cmdwindow.height
    ...
    cmdwindow.y = heightCMD - cmdwindow.height # fuck it
    case side
    when :left
...
def getSkinColor(windowskin, color, isDarkSkin)
  if !windowskin || windowskin.disposed? ||
    ...
    textcolors = [
      [34, 169, 255], [72, 80, 88],   # 1  Blue
      [255,  52,  52], [72, 80, 88],   # 2  Red
      [ 82, 255,  52], [72, 80, 88],   # 3  Green
      [ 72, 216, 216], [72, 80, 88],   # 4  Cyan
      [208,  56, 184], [72, 80, 88],   # 5  Magenta
      [228, 194,  85], [72, 80, 88],   # 6  Yellow
      [160, 160, 168], [72, 80, 88],   # 7  Gray
      [240, 240, 248], [72, 80, 88],   # 8  White
      [114,  64, 232], [72, 80, 88],   # 9  Purple
      [200, 96,  16], [72, 80, 88],   # 10 Orange
    ...
    # end
    # Light background, dark text
    return shadowc3tag(textcolors[2 * (color - 1)], MessageConfig::DARK_TEXT_SHADOW_COLOR_ALT)
...
def pbSetSystemFont(bitmap)
  ...
  bitmap.font.size = 22 if $PokemonSystem.language == 2
end
