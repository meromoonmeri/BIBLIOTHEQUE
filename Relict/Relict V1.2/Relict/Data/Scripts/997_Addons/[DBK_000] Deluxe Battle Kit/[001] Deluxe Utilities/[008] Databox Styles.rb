...
GameData::DataboxStyle.register({
  ...
  :offset_y      => [[-76, -8, 0, 38], [-80, -10, -34, 36, 12, 82]],
...
class Battle::Scene::PokemonDataBox
  ...
  def set_style_properties(sideSize)
    ...
    @show_hp_numbers = (@battler.index.even?) ? true : false
    @displayPos   = {
  ...
  def draw_style_text
    ...
    if @battler.index.even?
      ...
      # textpos.push(["#{@battler.hp} / #{@battler.totalhp}", 100, 50, :center, STYLE_BASE_COLOR, STYLE_SHADOW_COLOR, true])
    elsif 
      if !@battler.wild?
      ...
      # Set base x position for gender and level (right of name)
      base_x = namePos[0] + name_width + 8
      base_y = namePos[1]

      # Gender symbol
      case @battler.gender
      when 0 # Male
        textpos.push(["♂", base_x, base_y, :left, MALE_BASE_COLOR, STYLE_SHADOW_COLOR, true])
        base_x += self.bitmap.text_size("♂").width + 8
      when 1 # Female
        textpos.push(["♀", base_x, base_y, :left, FEMALE_BASE_COLOR, STYLE_SHADOW_COLOR, true])
        base_x += self.bitmap.text_size("♀").width + 8
      end

      # Level
      textpos.push(["Lv.#{@battler.level}", base_x, base_y, :left, STYLE_BASE_COLOR, STYLE_SHADOW_COLOR, true])
      end
