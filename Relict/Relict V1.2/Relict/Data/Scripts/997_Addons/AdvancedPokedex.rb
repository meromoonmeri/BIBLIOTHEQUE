...
if !PluginManager.installed?("Advanced Pokédex")
  ...
  class PokemonPokedexInfo_Scene
    ...
    def pbStartScene(dexlist,index,region)
      ...
      @maxPage = ADVANCED_PAGE# $game_switches[SWITCH] ? ADVANCED_PAGE : ADVANCED_PAGE-1
      @subPage=1
    ...
    def drawPageAdvanced
      @sprites["backgroundBG"].setBitmap(_INTL("Graphics/UI/Pokedex/advancedPokedexBG"))
    ...
    def displaySubPage
      ...
      overlay.clear
      line2Y = OLD_HEIGHT-54
      
      # Bottom text  
      textpos = [[
        @data.name,OLD_WIDTH/2,OLD_HEIGHT-82,2,BASE_COLOR,SHADOW_COLOR
      ]]
      if $player.owned?(@species)
        textpos.push([
          _INTL("{1}/{2}",@subPage,@totalSubPages),
          OLD_WIDTH-52,OLD_HEIGHT-52,1,BASE_COLOR,SHADOW_COLOR
        ])
      end
      ...
      # Type icon
      type1rect = Rect.new(
        0,GameData::Type.get(@data.types[0]).icon_position*28,64,28
      )
      type2rect = @data.types.size>1 ? Rect.new(
        0,GameData::Type.get(@data.types[1]).icon_position*28,64,28
      ) : nil 
