...
class Battle::Scene  
  ...
  def pbToggleMoveInfo(*args)
    ...
    (@sprites["enhancedUI_BG"].visible) ? pbHideUIPrompt : pbRefreshUIPrompt
    pbUpdateTargetIcons
  ...
  def pbUpdateTargetIcons
    ...
    @battle.allBattlers.each do |b|
      if b && !b.fainted? && b.index.odd?
        ...
        @sprites["info_icon#{b.index}"].x = 512 + 32 -(idx * 64)# Graphics.width - 32 - (idx * 64)
        @sprites["info_icon#{b.index}"].y = 96 # 48
        if b.dynamax?
  ...
  def pbUpdateMoveInfoWindow(battler, specialAction, cw)
    ...
    drawTextEx(@enhancedUIOverlay, xpos + 8, ypos + 74, 512 - 12, 2, 
  ...
  def pbDrawTypeEffectiveness(xpos, ypos, move, type, imagePos)
    ...
    @battle.allBattlers.each do |b|
      ...
      if b && !b.fainted? && move.category < 2
        ...
        imagePos.push([@path + "move_effectiveness", 512 - 64 - (idx * 64), ypos - 76, effct * 64, 0, 64, 76])
