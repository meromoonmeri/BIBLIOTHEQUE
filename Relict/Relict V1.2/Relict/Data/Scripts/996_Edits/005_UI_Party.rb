...
class PokemonPartyBlankPanel < Sprite
  ...
  def initialize(_pokemon, index, viewport = nil)
    super(viewport)
    self.x = (index % 2) * 512 / 2 #Graphics.width originally
...
class PokemonParty_Scene
  def pbStartScene(party, starthelptext, annotations = nil, multiselect = false, can_access_storage = false)
    ...
    @viewportBG.z = 99999
    @viewport = Viewport.new(PARTY_UI_X, PARTY_UI_Y, Graphics.width, Graphics.height)
    @viewport.z = 99999
    @viewportWindow = Viewport.new(0, 0, Graphics.width, Graphics.height)
    @viewportWindow.z = 99999

    @multiselect = multiselect
    @can_access_storage = can_access_storage
    addBackgroundPlane(@sprites, "partybg", "Party/bg", @viewportBG)
    @sprites["messagebox"] = Window_AdvancedTextPokemon.new("")
    @sprites["messagebox"].z              = 50
    @sprites["messagebox"].viewport       = @viewportWindow
    ...
    @sprites["storagetext"].viewport    = @viewportWindow
    ...
    @sprites["helpwindow"].viewport = @viewportWindow
...
MenuHandlers.add(:party_menu, :mail, {
  "name"      => _INTL("Mail"),
  "order"     => 40,
  "condition" => proc { |screen, party, party_idx| next party[party_idx] && !party[party_idx].egg? && party[party_idx].mail },
  "effect"    => proc { |screen, party, party_idx|
    pkmn = party[party_idx]
